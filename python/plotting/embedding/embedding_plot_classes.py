...
class single_plotline:
    def __init__(self,
        ...
        self.nick = num_file.replace("/", "_").replace(".root", "").replace(
            "?", "").replace("*", "") + "_" + num_folder + "_" + num_tree
    ...
    def return_nevents(self):
        ...
        for event in tree:
            if event.diLepMass > 40.0 and event.diLepMass <= 140.0:
                nweighted_events += event.eventWeight
    ...
    def clone(self,
        ...
        cloned_plotline = single_plotline(
            ...
            legend_marker=self.legend_marker
            if legend_marker is None else legend_marker,
...
=======
    def __init__(self,
                 ...
                 horizontal_subplot_lines=[],
                 add_nicks=None,
                 add_result_nicks=None,
                 add_scale_factors=None,
                 nicks_blacklist=None,
                 plotlines=[]):
        ...
        self.vertical_lines = vertical_lines
        self.horizontal_subplot_lines = horizontal_subplot_lines
        self.add_nicks = add_nicks
        self.add_result_nicks = add_result_nicks
        self.add_scale_factors = add_scale_factors
        self.nicks_blacklist = nicks_blacklist
        self.plotlines = plotlines
        self.out_json = jsonTools.JsonDict({})
    ...
    def clone(self,
        ...
        cloned_plot = single_plot(
            ...
            if x_expression is None else x_expression,
            y_expression=self.y_expression
            if y_expression is None else y_expression,
            y_lims=self.y_lims if y_lims is None else y_lims,
            z_lims=self.z_lims if z_lims is None else z_lims,
            z_label=self.z_label if z_label is None else z_label,
            x_label=self.x_label if x_label is None else x_label,
            ...
            z_log=self.z_log if z_log is None else z_log,
            weight=self.weight if weight is None else weight,
            normalized_to_nevents=self.normalized_to_nevents
            if normalized_to_nevents is None else normalized_to_nevents,
            normalized_to_unity=self.normalized_to_unity
            if normalized_to_unity is None else normalized_to_unity,
            normalized_to_hist1=self.normalized_to_hist1
            if normalized_to_hist1 is None else normalized_to_hist1,
            normalized_by_binwidth=self.normalized_by_binwidth
            if normalized_by_binwidth is None else normalized_by_binwidth,
            normalize_reference=self.normalize_reference
            if normalize_reference is None else normalize_reference,
            normalize_targets=self.normalize_targets
            if normalize_targets is None else normalize_targets,
            add_overflow_bin=self.add_overflow_bin
            if add_overflow_bin is None else add_overflow_bin,
            profiled=self.profiled if profiled is None else profiled,
            stacked=self.stacked if stacked is None else stacked,
            plot_type=self.plot_type if plot_type is None else plot_type,
            subplot_denominator=self.subplot_denominator
            if subplot_denominator is None else subplot_denominator,
            subplot_numerators=self.subplot_numerators
            if subplot_numerators is None else subplot_numerators,
            y_subplot_label=self.y_subplot_label
            if y_subplot_label is None else y_subplot_label,
            y_subplot_lims=self.y_subplot_lims
            if y_subplot_lims is None else y_subplot_lims,
            export_root=self.export_root
            if export_root is None else export_root,
            print_infos=self.print_infos
            if print_infos is None else print_infos,
            errorband_lines=self.errorband_lines
            if errorband_lines is None else errorband_lines,
    ...
    def return_json_with_changed_x_and_weight(self,
                                              x_expressions=[],
                                              weights=[]):
        ret_json_list = []
        if len(x_expressions) == len(weights):
            for i in range(len(x_expressions)):
                akt_plot = self.clone(
                    x_expression=x_expressions[i], weight=weights[i])
                akt_plot.fill_single_json()
                ret_json_list.append(akt_plot.out_json)
        elif weights == []:
            for akt_x_expression in x_expressions:
    ...
    def safe_append_modules(self, modulename, moduletype):
        try:
            count = self.out_json.get(moduletype + "_modules").count(
                modulename)
            if count == 0:
                self.out_json.setdefault(moduletype + "_modules",
                                         []).append(modulename)
        except:
            self.out_json.setdefault(moduletype + "_modules",
                                     []).append(modulename)
    ...
    def create_subplot(self):
        if self.add_nicks != None:
            sub_add_nick = " "
            for index in self.add_nicks:
                if self.plot_type == "efficiency":
                    sub_add_nick += self.plotlines[index].eff_nick + " "
                elif self.plot_type == "absolute":
                    sub_add_nick += self.plotlines[index].num_nick + " "
                else:
                    print "No proper plot type defined. Choose 'efficiency' or 'absolute'."
                    sys.exit()
        ...
        if self.plot_type == "efficiency":
            ...
            sub_den_nick = self.plotlines[self.subplot_denominator].num_nick
        else:
            print "No proper plot type defined. Choose 'efficiency' or 'absolute'."
            sys.exit()
        try:
        ...
        #   sub_den_nick = ""
        #   if self.plot_type == "efficiency":
        #           sub_den_nick = self.plotlines[index].eff_nick
        #   elif self.plot_type == "absolute":
        #       sub_den_nick = self.plotlines[index].num_nick
        #   else:
        #       print "No proper plot type defined. Choose 'efficiency' or 'absolute'."
        #       sys.exit()
        #   self.out_json.setdefault("divide_denominator_nicks", []).append(sub_den_nick)
        for index in self.subplot_numerators:
            sub_num_nick = ""
            if self.plot_type == "efficiency":
                sub_num_nick = self.plotlines[index].eff_nick
            elif self.plot_type == "absolute":
                sub_num_nick = self.plotlines[index].num_nick
            else:
                print "No proper plot type defined. Choose 'efficiency' or 'absolute'."
                sys.exit()
            ...
            self.out_json.setdefault("divide_numerator_nicks",
                                     []).append(sub_num_nick)
            self.out_json.setdefault("divide_result_nicks",
                                     []).append("div_" + sub_num_nick)
            self.out_json.setdefault("subplot_nicks",
                                     []).append("div_" + sub_num_nick)

            self.out_json.setdefault("colors", []).append(
                self.plotlines[index].color)
            self.out_json.setdefault("labels", []).append(
                self.plotlines[index].label)
            if "P" not in self.plotlines[index].marker:
                self.plotlines[index].marker = "E2"
            self.out_json.setdefault("markers", []).append(
                self.plotlines[index].marker)
    ...
    def fill_single_json(self):
        ...
        self.out_json.setdefault("x_expressions", []).append(self.x_expression)
        if not self.y_expression is None:
            self.out_json.setdefault("y_expressions", []).append(
                self.y_expression)
        if not self.y_lims is None: self.out_json["y_lims"] = self.y_lims
        if not self.y_bins is None: self.out_json["y_bins"] = self.y_bins
        if not self.z_lims is None: self.out_json["z_lims"] = self.z_lims
        ...
        self.out_json["z_log"] = self.z_log
        self.out_json.setdefault("weights", []).append(self.weight)
        # if self.nicks_blacklist != None:
        #     for index in self.nicks_blacklist:
        #         self.out_json.setdefault("nicks_blacklist", []).append(
        #             self.plotlines[index].num_nick)
        # making the plot out of available files both for efficiency and absolute plot type
        if not self.normalize_reference is None and self.normalize_targets != [] and self.plot_type == "absolute":
            reference_events = self.plotlines[
                self.normalize_reference].return_nevents()
            for plot_index in self.normalize_targets:
                target_events = self.plotlines[plot_index].return_nevents()
                self.plotlines[plot_index].scale_factor = float(
                    reference_events) / float(target_events)
        for akt_plotline in self.plotlines:
            if self.x_expression == "NKappaPackedPFCandidates":
                akt_plotline.calculate_mean_and_rms("NKappaPackedPFCandidates")
            if akt_plotline.num_file == None or akt_plotline.num_folder == None or akt_plotline.num_tree == None:
                print "Numerator is not properly set for line {LINE}. Need file, folder and tree".format(
                    LINE=akt_plotline.name)
                sys.exit()
            ...
            self.out_json.setdefault("colors", []).append(akt_plotline.color)
            self.out_json.setdefault("legend_markers", []).append(
                akt_plotline.legend_marker)

            if self.stacked:
                self.out_json.setdefault("stacks", []).append(
                    akt_plotline.stack)

            if self.plot_type == "efficiency":
                if akt_plotline.den_file == None or akt_plotline.den_folder == None or akt_plotline.den_tree == None:
                    print "Denominator is not properly set for line {LINE}. Need file, folder and tree".format(
                        LINE=akt_plotline.name)
                    sys.exit()

                self.out_json.setdefault("scale_factors", []).append(
                    akt_plotline.scale_factor)
                self.out_json.setdefault("files", []).append(
                    akt_plotline.num_file)
                ntree = "/" + akt_plotline.num_tree if not akt_plotline.num_tree == "" else ""
                self.out_json.setdefault(
                    "folders", []).append(akt_plotline.num_folder + ntree)
                self.out_json.setdefault("nicks", []).append(
                    akt_plotline.num_nick)
                # self.out_json.setdefault("nicks_blacklist", []).append(
                #     akt_plotline.num_nick)

                self.out_json.setdefault("files", []).append(
                    akt_plotline.den_file)
                dtree = "/" + akt_plotline.den_tree if not akt_plotline.den_tree == "" else ""
                self.out_json.setdefault(
                    "folders", []).append(akt_plotline.den_folder + dtree)
                self.out_json.setdefault("nicks", []).append(
                    akt_plotline.den_nick)
                # self.out_json.setdefault("nicks_blacklist", []).append(
                #     akt_plotline.den_nick)

                self.safe_append_modules(
                    modulename="Efficiency", moduletype="analysis")
                self.out_json.setdefault("efficiency_numerator_nicks",
                                         []).append(akt_plotline.num_nick)
                self.out_json.setdefault("efficiency_denominator_nicks",
                                         []).append(akt_plotline.den_nick)
                self.out_json.setdefault("efficiency_nicks", []).append(
                    akt_plotline.eff_nick)

            elif self.plot_type == "absolute":
                if self.normalized_to_nevents == True:
                    self.out_json.setdefault("scale_factors", []).append(
                        akt_plotline.calculate_nevents_scalefactor())
                else:
                    self.out_json.setdefault("scale_factors", []).append(
                        akt_plotline.scale_factor)
                self.out_json.setdefault("files", []).append(
                ...
                self.out_json.setdefault("nicks", []).append(
                    akt_plotline.num_nick)
                if self.normalized_to_unity == True:
                    self.safe_append_modules(
                        modulename="NormalizeToUnity", moduletype="analysis")
                if self.normalized_to_hist1 == True:
                    self.safe_append_modules(
                        modulename="NormalizeToFirstHisto",
                        moduletype="analysis")
                if self.normalized_by_binwidth == True:
                    self.safe_append_modules(
                        modulename="NormalizeByBinWidth",
                        moduletype="analysis")
                if self.add_overflow_bin == True:
                    self.safe_append_modules(
                        modulename="AddOverflowBin", moduletype="analysis")
            else:
                print "No proper plot type defined. Choose 'efficiency' or 'absolute'."
                sys.exit()
        for hline_y in self.horizontal_lines:
        ...
        for vline_x in self.vertical_lines:
            self.out_json.setdefault("vertical_lines", []).append(vline_x)
        # making a subplot with ratios, when denominator and numerators are given, both for 'efficiency' and 'absolute' plot type
        if self.subplot_denominator != None and len(
            ...
            for hline_y in self.horizontal_subplot_lines:
                self.out_json.setdefault("subplot_lines", []).append(hline_y)
        if self.export_root == True:
            self.safe_append_modules(
                modulename="ExportRoot", moduletype="plot")
        if self.print_infos == True:
            self.safe_append_modules(
                modulename="PrintInfos", moduletype="analysis")
        if len(self.errorband_lines) == 2:
            self.safe_append_modules(
                modulename="Errorband", moduletype="analysis")
            nick_list = []
            for index in self.errorband_lines:
                nick_list.append(self.plotlines[index].num_nick)
            self.out_json.setdefault("errorband_histogram_nicks", []).append(
                " ".join(nick_list))
            self.out_json.setdefault("errorband_result_nicks", []).append(
                "eb_" + self.plotlines[self.errorband_lines[0]].num_nick)

        # Plots with Error Band for first plot in red

        if "MvaIDEB1" in self.x_expression:
            self.out_json["texts_below_legend"]=["|#eta| < 0.8"]
            #~ self.out_json["texts"]=["|#eta|<0.8"]
            #~ self.out_json["texts_x"]=[0.56]
            #~ self.out_json["texts_y"]=[0.56]
            self.out_json["exclude_before"]=0.95
        elif "MvaIDEB2" in self.x_expression:
            self.out_json["texts_below_legend"]=["0.8 < |#eta| < 1.479"]
            #~ self.out_json["texts_x"]=[0.63]
            #~ self.out_json["texts_y"]=[0.56]          
            self.out_json["exclude_before"]=0.95

        elif "MvaIDEE" in self.x_expression:
            self.out_json["texts_below_legend"]=["|#eta| > 1.479"]
            self.out_json["exclude_before"]=0.95

            #~ self.out_json["texts_x"]=[0.58]
            #~ self.out_json["texts_y"]=[0.56]          
        #~ self.out_json["colors"] = ["kRed kWhite", "kGreen+1 kWhite", "kBlue", "kWhite kRed", "kRed", "kGreen+1 kWhite","kBlue"]
        #~ self.out_json["marker_styles"] = [20, 20,20,0, 0, 20, 20]
        #~ self.out_json["markers"] = ["HIST","HIST","PE","E2", "E2", "HIST","PE"]
        #~ self.out_json["fill_styles"] = [1001,1001, 1001, 3001, 3001, 1001, 1001]
        #~ self.out_json["legend_markers"] = ["L","L","PE","F","L","L","PE"]       
        self.out_json["colors"] = ["kRed kWhite", "kRed", "kBlue", "kRed" ,"kBlue"]
        ...
        self.out_json["cms"] = True
        self.out_json["scale_factors"] = [0.001]
        self.out_json["extra_text"] = "Simulation Preliminary"        
       
        self.out_json["legend_markers"] = ["","FLP","PEL","L","","PE"]
        self.out_json["nicks_blacklist"] = ["noplot"]
        # Plots with UP/Down Tau Variations
        if any(string in self.x_expression for string in ["iso_", "nbtag", "njet", "eSuperCluster","hadronicOverEm","hcal1Iso_1","hcal2Iso_1"]):
            self.out_json["scale_factors"] = [1]
            self.out_json["y_label"] = "N_{evts}"
            self.out_json["y_log"] = "True"
            self.out_json["y_rel_lims"] = [0.5,20.0]
            if "iso_2" in self.x_expression:
                self.out_json["exclude_after"]=0.1
            if "iso_1" in self.x_expression:
                self.out_json["exclude_after"]=0.1
        try:
            if False:
                self.out_json["colors"] = ["kRed kWhite", "kWhite kRed", "kMagenta kWhite" ,"kMagenta kWhite",  "kBlue", "kRed", "kMagenta kWhite","kMagenta kWhite", "kBlue"]
                self.out_json["marker_styles"] = [20, 0, 20, 20, 20, 0, 20, 20, 20]
                self.out_json["markers"] = ["HIST", "E2","HIST", "HIST", "PE", "E2", "HIST", "HIST", "PE"]
                self.out_json["fill_styles"] = [1001,  3001,1001, 1001, 1001, 3001, 1001, 1001, 1001]
                self.out_json["legend_markers"] = ["L","F","L", "", "PE","L","L", "L", "PE"]
        except IndexError:
            pass
>>>>>>> Stashed changes
